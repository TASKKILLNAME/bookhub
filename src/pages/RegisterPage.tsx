...
import { BookOpen, UserPlus, Eye, EyeOff, Mail, CheckCircle } from 'lucide-react';
...
function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setError('');
    if (!email.trim() || !password.trim() || !name.trim()) { setError('모든 필드를 입력해주세요.'); return; }
    ...
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: 'calc(100vh - 160px)', padding: 16 }}>
        <div className="card" style={{ width: '100%', maxWidth: 440, padding: 40, textAlign: 'center' }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: 64, height: 64, borderRadius: '50%', background: 'rgba(63, 185, 80, 0.15)', marginBottom: 20 }}>
            <Mail size={32} color="#3fb950" />
          </div>
          <h1 style={{ fontSize: '1.5rem', fontWeight: 700, color: '#e6edf3', marginBottom: 10 }}>
            이메일을 확인해주세요
          </h1>
          <p style={{ color: '#8b949e', fontSize: '0.95rem', lineHeight: 1.7, marginBottom: 8 }}>
            <strong style={{ color: '#e6edf3' }}>{email}</strong>
            <br />으로 인증 링크를 보냈습니다.
          </p>
          <p style={{ color: '#6e7681', fontSize: '0.85rem', lineHeight: 1.6, marginBottom: 28 }}>
            메일함에서 인증 링크를 클릭하면
            <br />자동으로 가입이 완료됩니다.
          </p>

          <div style={{ display: 'flex', flexDirection: 'column', gap: 10, alignItems: 'center' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 20px', borderRadius: 8, backgroundColor: 'rgba(63, 185, 80, 0.08)', border: '1px solid rgba(63, 185, 80, 0.2)', fontSize: '0.85rem', color: '#3fb950' }}>
              <CheckCircle size={16} />
              스팸 폴더도 확인해보세요
            </div>
            <Link to="/login" style={{ color: '#58a6ff', textDecoration: 'none', fontSize: '0.9rem', fontWeight: 500, marginTop: 8 }}>
              로그인 페이지로 이동
            </Link>
          </div>
        </div>
    ...
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: 'calc(100vh - 160px)', padding: 16 }}>
      <div className="card" style={{ width: '100%', maxWidth: 400, padding: 32 }}>
        <div style={{ textAlign: 'center', marginBottom: 32 }}>
          ...
          <h1 style={{ fontSize: '1.5rem', fontWeight: 700, color: '#e6edf3', marginBottom: 6 }}>회원가입</h1>
          <p style={{ color: '#8b949e', fontSize: '0.9rem' }}>독서 여정을 시작하세요</p>
        </div>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div>
            ...
            <input type="text" className="input-field" placeholder="표시될 이름을 입력하세요" value={name} onChange={(e) => setName(e.target.value)} />
          </div>
          <div>
            <label style={{ display: 'block', fontSize: '0.85rem', fontWeight: 500, color: '#e6edf3', marginBottom: 6 }}>비밀번호</label>
            <div style={{ position: 'relative' }}>
              <input type={showPassword ? 'text' : 'password'} className="input-field" style={{ paddingRight: 44 }} placeholder="6자 이상 입력하세요" value={password} onChange={(e) => setPassword(e.target.value)} autoComplete="new-password" />
          ...
          <div>
            <label style={{ display: 'block', fontSize: '0.85rem', fontWeight: 500, color: '#e6edf3', marginBottom: 6 }}>비밀번호 확인</label>
            <input type="password" className="input-field" placeholder="비밀번호를 다시 입력하세요" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} autoComplete="new-password" />
          </div>
          <button type="submit" className="btn-primary" disabled={isLoading} style={{ width: '100%', marginTop: 4, opacity: isLoading ? 0.7 : 1 }}>
            {isLoading ? '가입 중...' : <><UserPlus size={18} /><span>회원가입</span></>}
          </button>
        </form>
        ...
        <p style={{ textAlign: 'center', marginTop: 20, fontSize: '0.9rem', color: '#8b949e' }}>
          이미 계정이 있으신가요?{' '}
          <Link to="/login" style={{ color: '#58a6ff', textDecoration: 'none', fontWeight: 600 }}>로그인</Link>
        </p>
      </div>
    </div>
  );
